...
When the wave lands or stalls, deliver one document to the human as a message. If the project keeps process records, also commit it alongside the plan file — in the wave's final PR, after the battery converges but before attestation, so the attested hash covers it, recorded with a post-review delta note (see the review-gate doc for what a delta note must contain). The committed copy must match the delivered message in substance — committing something other than what the human read defeats the report's purpose and is treated as artifact fabrication. Note the report is never itself leg-reviewed: that is a wider unreviewed-prose window than a fix delta, accepted because the report is by design the document the human actually reads. Do not put reports in the hash-exempt artifacts path: that zone is for machine-checked attestations, and the report is a document humans act on. Order the report:
