...
battery legs, that each leg was produced by an allowlisted model, and that
no validated blocking finding (security critical/high, review P1) remains
unfixed. Judges that the judging happened — never the code itself. Stdlib
only; runs identically in CI and locally.

Ported from the somnus implementation (kristovatlas/somnus, dev branch).

Usage:
  python scripts/review_gate.py --pr 2 --base origin/main
  python scripts/review_gate.py --hash-only --base origin/main
"""
...
# Model floor (Kristov, 2026-07-21), enforced deterministically: each leg's
# model must be on its vendor's allowlist. "Or better" is admitted by adding
# the new model id here — via a gated PR, so the floor itself cannot drift
# silently. Keyed by leg-name prefix up to the first hyphen.
ALLOWED_MODELS: dict[str, set[str]] = {
    "claude": {"claude-opus-4-8", "claude-fable-5"},
    "codex": {"gpt-5.6-sol"},
}
...
def check_artifact(path: Path, leg: str, leg_type: str, expected_hash: str) -> list[str]:
    ...
    vendor = leg.split("-", 1)[0]
    allowed = ALLOWED_MODELS.get(vendor, set())
    if isinstance(data.get("model"), str) and data["model"] not in allowed:
        fails.append(
            f"{path.name}: model {data['model']!r} not in the {vendor} "
            f"allowlist {sorted(allowed)} — the model floor is enforced; "
            f"add new models via a gated PR editing ALLOWED_MODELS"
        )
    ...
    for i, f in enumerate(data["findings"]):
        ...
        # (fail-closed but a traceback, not a diagnostic).
